...
export const getDatabase = async (slug?: string) => {
  ...
  return response.results.map((result) => {
    ...
    const title =
      ...
      properties.Title.title.slice(-1)[0]?.plain_text;
    const date = properties.Date.type === "date" && properties.Date.date?.start;
    const updateDate =
      properties.UpdateDate.type === "last_edited_time" &&
      properties.UpdateDate.last_edited_time;

    const undefinedCover =
    ...
    const category = properties.Category.type === "select" && {
      name: properties.Category.select?.name,
      color: properties.Category.select?.color,
    };
    const tags =
    ...
    const series = properties.Series.type === "select" &&
      properties.Series.select?.name !== undefined && {
        name: properties.Series.select?.name,
        color: properties.Series.select?.color,
      };
    const excerpt =
      properties.Excerpt.type === "rich_text" &&
      properties.Excerpt.rich_text[0]?.plain_text;
    const tip =
      properties.Tip.type == "rich_text" &&
      properties.Tip.rich_text[0]?.plain_text;
    const sspai = properties.SSPAI.type == "url" && properties.SSPAI.url;
    const originalCover =
      properties.OriginalCover.type == "checkbox" &&
      properties.OriginalCover.checkbox;
    const colorTitle =
      properties.ColorTitle.type == "checkbox" &&
      properties.ColorTitle.checkbox;
    return {
      id: result.id,
      slug: slug || "",
      title: title || "",
...
export const getAllBlocks = async (parentId: string) => {
  ...
  const result = Promise.all(
    blocks.map((b: any) => {
      ...
      liteBlocks[liteBlocks.type]["text"] &&
        liteBlocks[liteBlocks.type]["text"].map((t: any) => {
          delete t["plain_text"];
          delete t["href"];
        });
