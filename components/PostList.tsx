    ...
    big:
      filter == undefined
        ? {
            ...
            title:
              "line-clamp-2 text-2xl md:(text-xl leading-tight line-clamp-3) lg:text-3xl",
          }
        : {
            cardSize:
              "h-90 w-full col-span-6 md:(col-span-3 h-85) lg:(h-90 col-span-6 flex flex-row)",
            imgSize: "h-3/5 md:h-5/9 lg:(h-full w-160)",
            title:
              "line-clamp-2 md:line-clamp-5 lg:line-clamp-6 text-xl leading-tight md:(text-lg leading-tight) lg:text-3xl",
          },
    medium: {
      cardSize: "col-span-6 md:(col-span-3 h-85) h-90 lg:h-107",
      imgSize: "h-3/5 md:h-5/9 lg:h-3/5",
      title:
        "line-clamp-2 text-xl leading-tight md:(text-lg leading-tight) lg:(text-2xl leading-tight)",
    },
    small: {
    ...
    <>
      <ListLayout>
        <div
          className="pb-10"
          data-aos="fade-up"
          data-aos-delay={filter == undefined ? "250" : "0"}
        >
          {filter == undefined ? (
            ""
          ) : (
          ...
          <h1
            className="mb-4 text-2xl font-bold md:text-3xl lg:mb-8"
            //  data-aos="fade-up"
          >
            {filter == undefined ? (
              "Latest Posts💫"
                ...
                >
                  <Link href="/post/[slug]" as={`/post/${post.slug}`}>
                    ...
                    md:hover:shadow-lg hover:rotate-0 hover:active:scale-95`}
                      ...
                      >
                        <header
                          className={`relative ${size.imgSize} duration-500 ease-in-out md:(filter group-hover:brightness-90) transition`}
                        >
                          <ThemedImage
                            post={post}
                            quality={100}
